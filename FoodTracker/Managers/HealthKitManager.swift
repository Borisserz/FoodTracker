...
@Observable final class HealthKitManager {
    ...
    func requestAuthorization() async throws {
        ...
        guard let bodyMass = HKObjectType.quantityType(forIdentifier: .bodyMass),
              ...
              let activeEnergy = HKObjectType.quantityType(forIdentifier: .activeEnergyBurned) // ✅ 1. ДОБАВИЛИ ТИП КАЛОРИЙ
        else { return }
        
        // ✅ 2. ДОБАВИЛИ activeEnergy В МАССИВ НА ЧТЕНИЕ
        let typesToRead: Set = [bodyMass, steps, activeEnergy]
        
        // Пишем воду и потребленные калории (еду)
    ...
    // ✅ НОВЫЙ МЕТОД ДЛЯ ШАГОВ (Без блокировки и с улучшенным временем)
    func fetchSteps(for date: Date) async throws -> Int {
        ...
        // 🔥 ИСПРАВЛЕНИЕ: Устанавливаем конец дня на полночь следующего дня
    ...
    func fetchTotalActiveCalories(for date: Date) async throws -> Int {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let query = HKStatisticsQuery(quantityType: energyType, quantitySamplePredicate: predicate, options: .cumulativeSum) { _, result, error in
                ...
                // Apple возвращает калории в килокалориях (килокалория = kcal)
