...
struct BurnedCaloriesRing: View {
    let burned: Int
    // Цель по активности: берем 25% от базового метаболизма как цель сжигания, или можно поставить дефолтные 500
    let targetBurn: Int
...
// MARK: - 3. КОЛЬЦО БАЛАНСА (NET CALORIES)
struct NetCaloriesRing: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            VStack(spacing: 6) {
                // Если Net отрицательный (сожгли больше чем съели), показываем с минусом
                Text("\(net)")
                    .font(.system(size: 52, weight: .heavy, design: .rounded))
